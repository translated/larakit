...
from typing import Optional, List, Union, ContextManager, Any
...
class PipelineActivity:
    ...
    def __init__(self, args, extra_argv=None, input_path: str = None, output_path: str = None, wdir: str = None,
        ...
        if wdir is None:
            _wdir = _temp_dir = tempfile.mkdtemp(prefix='tmp_')
        else:
            _wdir = wdir
            _temp_dir = None
        ...
        # Resuming state
        self._state_file: str = os.path.join(self._wdir, 'state.json')
        self._steps: List['PipelineActivity.Step'] = self.steps()

        if os.path.isfile(self._state_file):
            with open(self._state_file, 'r', encoding='utf-8') as f_input:
                _state = Namespace(**json.load(f_input))
        else:
            _state = Namespace(step_no=-1)

        self.state: Namespace = _state
    ...
    def save_state(self) -> None:
        with open(self._state_file, 'w', encoding='utf-8') as f_output:
            f_output.write(json.dumps(self.state.to_json(), indent=2, sort_keys=True))
